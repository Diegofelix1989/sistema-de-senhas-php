...
try {
    ...
    if ($senha['atendimento_iniciado_em'] === null) {
        $pdo->rollBack();
        echo json_encode(['error' => 'O atendimento desta senha ainda não foi iniciado']);
        exit;
    }
    
    // Registrar o fim do atendimento e atualizar o status para 'atendido'
    ...
    $pdo->rollBack();
    echo json_encode(['error' => 'Erro ao finalizar atendimento: ' . $e->getMessage()]);
}